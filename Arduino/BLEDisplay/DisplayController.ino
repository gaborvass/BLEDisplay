...
void LED_PrintValueI(unsigned char x, unsigned char y, int data)
{
 ...
 char messageToDisplay[64] = {0};
 
 void clearMessage(){
   for(int i = 0; i < 64; ++i){
     messageToDisplay[i] = NULL;
   }
 }
 
 void setMessageToDisplay(char* newMessage){
   clearMessage();
   for(int i = 0; i < 64; ++i){
     messageToDisplay[i] = newMessage[i];
   }
 }
 
 
 void displayInt(int data){
    LED_PrintValueI(40, 2, data);   
 }
 
 void loopDisplay()
 {
    //LED_P8x16Str(23,0,"welcome to");
    LED_P8x16Str(40,2, messageToDisplay);
